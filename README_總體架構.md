...
使用者明確要求：人工標註暫緩期間改由 LLM（本對話 Claude Sonnet 5）直接閱讀文件並判斷用途類別，取代原訂 D-001 的「人工 20%＋規則弱標籤 80%」方案，用於訓練並展示分類模型。依既有決策規則（AGENTS.md「未來設計決策規則」），先查文獻再定標註準則。

| 項目 | 決策內容 |
|---|---|
| 問題 | LLM 直接對文件分類標註，應遵循什麼可驗證準則？能否視為訓練用途的合理標籤來源？ |
| 研究查核 | 查找 LLM 作為文字標註者（annotator）／LLM-as-judge 的信度與效度研究；查找弱監督（silver label）稽核抽樣建議 |
| 直接證據 | [Gilardi, Alizadeh & Kubli (2023) PNAS](https://www.pnas.org/doi/10.1073/pnas.2305016120)：ChatGPT 零樣本標註在四組文字分類任務上準確率平均高於群眾外包標註者約25個百分點，標註者間一致性（intercoder agreement）也優於群眾標註者；但作者同時指出成效依任務與資料集而異，非放諸四海皆準。[Are chatbots reliable text annotators? Sometimes (PMC)](https://pmc.ncbi.nlm.nih.gov/articles/PMC11954583/)：LLM 與人工標註者的一致性可與兩位人工標註者之間的一致性相當，但需用 Cohen's κ／Krippendorff's α 等既有人工標註信度標準衡量，不能只看表面準確率；LLM 自陳信心分數尚未被證實可靠，不能直接當校準機率使用。 |
| 弱監督稽核依據 | [Weak supervision/silver-label 稽核抽樣建議整理](https://medium.com/@khayyam.h/data-annotation-3-0-weak-supervision-and-self-labeling-for-faster-ground-truth-c99b79a3e21e) 與相關文獻：silver label 應視為代理標籤而非真值，多類別標註建議稽核樣本規模約50–100筆、依標籤分布分層抽樣；LLM-as-judge 相關綜述（[Patronus LLM-as-a-Judge 教學](https://www.patronus.ai/llm-testing/llm-as-a-judge)）建議標註準則需與人工共同制定、並抽樣比對人工結果，範例本身也可能帶入偏誤。 |
| 採用方案 | LLM 標註視為「弱監督/silver label」，比照既有 term_mapping 規則弱標籤的地位（[D-001](#d-001人工弱標籤訓練資料維持約28)），不視為人工真值：(1) 沿用既有 7 類定義與現有 review 匯入機制，reviewer 欄位明確標示 `claude-sonnet-5-llm`（與人工 reviewer `pososos` 區隔，稽核可追溯）；(2) 標註依來源（host／文件家族）分組，每類保留至少一個獨立來源整組作為訓練用 holdout 測試分組，避免同來源／近似文件跨 train/test 洩漏，沿用既有來源隔離原則；(3) 單輪判斷（讀標題＋前約1,200字預覽），未做多輪自我一致性或多模型交叉驗證，於 note 欄位記錄判斷依據；(4) 標註後以現有規則弱標籤（term_mapping）候選作獨立比對，回報一致率作為粗略稽核，不作為分類器準確率結論；(5) 既有 12 筆人工標籤完全不覆寫、不參與本輪判斷範圍，並保留作為與 LLM 訓練結果比較的獨立參照。 |
| 本地證據 | 尚無：本節僅記錄查核與流程設計，實際標註筆數、規則比對一致率、holdout 訓練成效將於下一節記錄。 |
| 重新評估條件 | 若規則弱標籤比對一致率明顯偏低、holdout 成效未優於既有人工基準、或使用者恢復人工標註，應重新檢視是否停用或縮限 LLM 標註範圍；沒有研究支持本案可用 LLM 標籤完全取代獨立人工測試集，最終成效驗證仍需真實代表性資料（[D-007](#d-007-選樣偏差修正2026-09-13)）。 |

此決策不修改 D-001～D-009 既有文字，屬暫緩期間的替代/補充方案；人工標註恢復後兩者標籤來源仍分開稽核，不自動合併視為同一信度等級。

## LLM 標註執行與訓練結果（2026-09-14）

依 D-010，Claude Sonnet 5 直接閱讀 240 份未標註 parsed 文件的標題與前約1,200字預覽，判斷七類用途並以既有 review 匯入機制寫入（`reviewer=claude-sonnet-5-llm`，與人工 `reviewer=pososos` 完全區隔，見 review_log）；2 份內容過短（僅品牌標語，無足夠判斷依據）予以跳過未標註。標註分布：experiment_log 91（Hannover 7 個疲勞試驗室之一，Lab.4／Lab.7 整組保留為測試分組，避免同實驗室資料跨 train/test）、research 38、manuscript_notes 36、safety 25、manual 22、industry 21、guidance 18。抽樣與既有規則弱標籤（term_mapping）重疊的 40 筆做比對，39 筆一致（97.5%）；唯一歧異為一份內政部建築研究所報告，規則判 industry、LLM 判 research，屬合理分歧（政府研究報告 vs 產業報告的邊界判斷）而非明顯錯誤。

依來源／文件家族分組，每類保留至少一組獨立來源（合計 23 個 source_id）作為測試集，訓練 170 筆、測試 81 筆，執行 `cement.train --holdout-sources`：**macro-F1 0.70（n=81）**。experiment_log／industry／safety／manuscript_notes 表現佳（F1 0.75–1.0）；guidance 與 research 在此次分組下互相混淆——guidance 測試 3 筆全部誤判為 research，research 測試 10 筆有 8 筆誤判為 guidance。原因可歸咎於本次分組巧合：guidance 訓練集全為 FHWA 英文技術報告、測試集全為 Sika 中文產品資料表；research 訓練集含 NIST 英文摘要與內政部中文研究報告、測試集全為 tsapps.nist.gov 純英文學術論文——兩類文字風格與語言分布在 train/test 間差異太大，字元 TF-IDF 模型難以泛化。這重現了 [D-008](#d-008-執行軌道一與軌道二首次結果2026-09-13) 所述「混合中英文不代表已具跨語言泛化能力」的警語，樣本量（n=81）遠大於先前 n=12 診斷，但分組方式是本輪選擇（非隨機或代表性抽樣），不能宣稱為正式跨語言成效數字。詳見 models/evaluation.json、models/registry.json（registry_id=20260913T171743Z-bfbfb587；時間戳為系統 UTC 時鐘，非本文件撰寫日期）。

另做一次獨立查核：僅用 LLM 標籤中屬原始五類（排除 experiment_log／manuscript_notes）的 112 筆訓練，測試對象為既有 12 筆人工標籤（`reviewer=pososos`，完全未參與本輪 LLM 標註）。結果 8/12 正確、macro-F1 0.62。manual 類別的 2 筆人工標籤全部誤判為 research——LLM 訓練集 manual 樣本全為實體檢測設備操作手冊（Humboldt／Giatec／Gilson），人工標籤 manual 樣本卻是 NIST 軟體工具手冊（VCCTL），兩者用詞差異大，暴露「manual」類別本身異質性（硬體設備手冊 vs 軟體工具手冊）。此查核**非**乾淨的獨立來源測試：nist.gov、fhwa.dot.gov、cementaustralia.com.au、heidelbergmaterials.com 同時出現在 LLM 訓練集與這 12 筆人工測試集中（僅 source_id 不同），且 n=12 過小；只能粗略反映 LLM 標籤是否與人工判斷方向一致，不是模型跨組織泛化能力的正式證據。詳見 data/reports/llm-label-human-verification-20260914.json。

模型已寫入 models/classifier.joblib；`cement.inference.infer(method='classifier')` 每次呼叫皆重新讀取檔案，demo_api／api 的「自訓分類」選項不需重啟服務即可反映新模型。已用 6 份保留測試文件現場呼叫驗證可正確推論（5/6 正確，唯一錯誤與正式評估相同屬 guidance/research 混淆案例）。既有 12 筆人工標籤全程未被覆寫、也未被計入本輪 LLM 標註範圍，兩者標籤來源持續分開稽核。

**誠實總結**：這是 LLM 直接標註＋來源隔離訓練首次在較大樣本（n=81／n=12 兩組獨立查核）下的量測，macro-F1 0.70／0.62 明顯優於 D-008 的 n=12 診斷（0.4），但測試分組由本輪選擇、非隨機代表性抽樣，且 guidance/research 混淆與 manual 類別異質性顯示明確弱點；不構成可信賴的正式分類成效數字，也不能取代 [D-007](#d-007-選樣偏差修正2026-09-13) 要求的真實代表性資料與獨立測試集。

## 四種分類策略同測試集頭對頭比較（2026-09-14，2026-09-14 修正證據效力說明）

使用者要求跟既有其他策略一起比較，才能判斷檢索＋重排是否有效。在完全相同的兩組測試文件上，同時跑四種既有／本輪方法：(A) 規則式弱標籤（term_mapping）、(B) 本輪 LLM 標籤訓練出的字元 TF-IDF 分類器、(C) 純檢索（E5-small embedding，標題→目錄→內容分階段，門檻 0.8／差距0.02）、(D) 檢索＋重排（同一串接，內容階段未過門檻才加 bge-reranker-v2-m3，門檻 0／差距1，與專案既有 D-006 已驗證門檻組合一致）。程式與完整結果見 data/reports/method-comparison-20260914.json。

**兩組測試集的證據效力不同，必須先分清楚，不能混在一起下結論**：

- **測試組2（12份既有獨立人工標籤，唯一有效證據）**：真值來自人工判讀（reviewer=pososos），與本輪 LLM 標註、四個方法的訓練／門檻設定完全無關，是唯一能拿來評斷「誰判斷得對」的資料。樣本量小（n=12）是唯一的限制。
- **測試組1（81份來源隔離 holdout，僅供觀察 B 的訓練擬合狀況，*不能*用來評斷 A／C／D 對不對）**：真值是本輪 LLM 自己標的標籤，不是獨立人工判讀。B 就是直接用這組標籤的訓練切分訓練出來的，「對」的定義等於「跟訓練標籤一致」，本質上是同義反覆。A／C／D 三個方法從未接觸過這組 LLM 標籤，它們跟 LLM 標籤不一致，可能是它們錯，也可能是 LLM 標籤本身錯（例如下方 guidance/research 混淆已知是 LLM 標籤與訓練切分造成，不是獨立驗證出的錯誤）——拿這組數字評斷 C／D 是循環論證，之前版本的本節誤把這組數字當作「準確率」使用，已修正下方表格用詞為「與LLM標籤一致率」並調整結論。

**測試組2（唯一有效證據，n=12）：**

| 方法 | 涵蓋率 | 有答案時準確率 | 整體準確率 | macro-F1 | 總耗時 |
|---|---|---|---|---|---|
| A 規則式 | 50%（6/12） | 100% | 50% | 0.57 | <1秒 |
| B LLM標籤訓練分類器（無洩漏版本，見上節查核） | 100%（12/12） | 66.7% | 66.7% | 0.62 | <1秒 |
| C 純檢索 | 67%（8/12） | 100% | 66.7% | 0.70 | 0.8秒 |
| **D 檢索＋重排** | **75%（9/12）** | 100% | **75%** | **0.76** | 42秒（4次重排） |

在這組唯一可信的證據上，**檢索＋重排（D）是四個方法中 macro-F1 與整體準確率最高的**，涵蓋率也最高，而且有答案時從未答錯（100% accepted accuracy，與C並列）；比起單純檢索（C），加上重排多接受了1份文件且答對了、多花約41秒。這是支持「檢索＋重排」這個方向的正面訊號。唯一該注意的是 n=12 太小——任何一筆預測翻轉就會讓數字跳動超過8個百分點，還不足以當作正式效能保證，需要更大規模的獨立人工測試集才能確立信心區間，但**目前唯一的有效證據沒有出現對檢索＋重排不利的訊號**。

**測試組1（僅供參考，不是有效證據，n=81）：**

| 方法 | 涵蓋率 | 與LLM標籤一致率（有答案時） | 總耗時(81份) |
|---|---|---|---|
| A 規則式 | 22%（18/81） | 100% | <1秒 |
| B LLM標籤訓練分類器 | 100%（81/81） | 81.5%（同義反覆，見上方說明） | <1秒 |
| C 純檢索 | 33%（27/81） | 81.5% | 9.8秒 |
| D 檢索＋重排 | 43%（35/81） | 65.7% | 546秒（54次重排） |

這組數字本身也有價值——它顯示 LLM 標籤與檢索/重排方法在這批文件上系統性地不一致（尤其 guidance／research 兩類，已在上節查核出是訓練切分的中英文風格差異造成），值得後續用真人覆核這些具體歧異案例，釐清究竟是 LLM 標籤錯還是檢索／重排錯；但在有更多獨立人工判讀之前，**不能引用這組數字說「檢索＋重排效果變差」**，先前版本這樣寫是誤用證據，已在此修正。

過程中另外發現並已修正一個真實 bug：程式最初直接沿用 `models/classifier.joblib` 評估測試組2的 B，但該模型的訓練集（170份）本來就包含這 12 筆人工文件（它們的 source_id 不在本輪 23 個 holdout 名單內），導致 100% 準確率毫無意義（模型訓練時已看過答案）；已改用「僅用五類 LLM 標籤訓練、完全排除這 12 筆」的無洩漏模型取代，即上方表格數字。

**結論**：在唯一有效的證據（測試組2）上，檢索＋重排目前是表現最好的方法，支持繼續投入這個方向；但 n=12 太小，不能當作正式效能保證，延遲成本（42秒／份，多數來自重排）也需要與更大樣本的準確率提升一起評估才能判斷是否划算。下一步應優先擴大測試組2這種「真人判讀、與訓練來源無關」的獨立樣本規模，而不是再靠 LLM 自行標註來驗證，這正是 [D-007](#d-007-選樣偏差修正2026-09-13) 一直在等的真實代表性資料。

## 全庫 parsed 文件標註完成與判定邏輯（2026-09-14）

使用者要求把全部檔案標註完成，並記錄判定邏輯。截至本節，**全庫 414 份 `status=parsed` 的文件已 100% 有標籤**（402 份 `reviewer=claude-sonnet-5-llm`＋12 份既有人工標籤 `reviewer=pososos`，兩者持續分開稽核，人工標籤全程未被覆寫）。標籤分布：research 134、experiment_log 91、safety 55、guidance 42、manuscript_notes 36、industry 31、manual 25。

**「全部檔案」的範圍界定**：這裡的「全部」指全庫中「可以被判斷」的文件，即已成功解析出文字的 414 份（`status=parsed`）。另有 99 份不在此範圍內：63 份 `out_of_scope`（明確排除，不應解析）、18 份 `needs_ocr`（掃描影像無文字層）、17 份 `needs_review`（解析出現亂碼或編碼異常，需人工核對文字品質）、1 份 `blocked_content`（下載到反爬蟲擋頁而非真正文件）。這 99 份沒有可信的文字可供判斷用途，不論人或 LLM 都無法憑空標註；要納入需要先解決 OCR／編碼／來源問題，屬於資料前處理的既有待辦，不是這次標註工作的一部分。

### 判定邏輯（單一標準：逐份閱讀「標題＋前約1,200字內容預覽」）

全部 402 份 LLM 標籤，不分批次，一律逐份閱讀「標題＋前約1,200字內容預覽」後獨立判斷，依 [用途分類契約表](#分類契約) 的定義套入七類之一：

- **research**：內容以研究問題、實驗方法、結果與文獻引用為主軸（含學術論文摘要頁、政府研究計畫報告、第三方實驗室依標準方法出具的正式試驗報告）。
- **manual**：內容是特定軟體或設備的操作說明（安裝、操作步驟、參數、故障排除），不論是實體儀器手冊還是軟體使用手冊。
- **guidance**：內容是工程選材、設計或施工的應用建議（含最佳實務指引、TechBrief、產品應用資料表），適用條件與限制是主體，不是研究方法本身。
- **safety**：GHS 格式安全資料表（危害分類、成分、處置與儲存），以「這是不是一份 SDS」為判準，不是「有沒有提到安全」。
- **industry**：企業年報、ESG／永續報告、產業現況報告，主體是組織、年度、指標與績效，不是技術方法。
- **experiment_log**：原始量測數據記錄（如感測器輸出的表格數據），主體是數字紀錄本身，不是對數據的分析或討論。
- **manuscript_notes**：課程講義、投影片、教學筆記，主體是教學呈現，不是正式出版的研究或指引。

唯一的例外是 Hannover 91 份疲勞試驗 TXT（只有 7 種不同的實驗室標頭格式，逐份閱讀等同重複讀同一種格式）：採「抽樣核對格式一致後批次判定」而非逐份全文閱讀——已於匯入前抽查全部 7 個實驗室各至少 1 份，確認欄位結構與內容性質一致才批次套用 `experiment_log`，並在 note 欄位註明「抽樣確認格式一致」而非宣稱逐份細讀。這是唯一刻意採用批次判定的類別，且已明確揭露依據。

**過程修正記錄（不迴避自己的方法論錯誤）**：標註實際分兩批匯入（前 251 份、後 163 份），後面這 163 份第一次匯入時，為了省事只讀了約350字並以「同host前批已大量抽樣過」為由跳過逐份獨立閱讀——這跟第一批「每份都讀1,200字」的深度不一致，被使用者指出後已用完全相同的 1,200 字深度重新逐份閱讀這 163 份，確認原標籤全部維持不變（含 2 個既有例外也再次確認成立），並更新資料庫裡的 note 欄位使其準確反映實際採用的方法。這個修正過程本身保留在此記錄，不是事後靜默改掉。

**判斷中發現的兩個「內容優先於來源」例外**（呼應本專案「分類依主要用途，不依副檔名或機構名稱」的既有設計準則，見上方「分類契約」一節）：

- 1 份 `www.nist.gov` 網頁實際是軟體使用手冊（"User's Guide to the NIST Virtual Cement and Concrete Testing Laboratory"）而非研究論文摘要 → 判 `manual` 而非 `research`。
- 1 份 `www.fhwa.dot.gov` 文件來自 Turner-Fairbank Highway Research Center 的 HRT 研究報告系列，內容是研究性質而非一般 HIF TechBrief 最佳實務指引 → 判 `research` 而非 `guidance`。

**已知限制（不迴避）**：
1. 所有 402 份 LLM 標籤都未經獨立人工覆核，只有跟既有 12 筆人工標籤的比對（97.5% 一致，Cohen's κ=0.96，但比對對象是規則引擎不是人）與跟 12 筆真人標籤的下游模型驗證（macro-F1 0.62，n=12）作為品質訊號，兩者都在上方章節說明過各自的證據力限制。
2. 即使逐份都讀了1,200字，這仍是單次、單一模型（Claude Sonnet 5）的判斷，沒有做多輪自我一致性或第二模型交叉驗證；文獻建議的稽核作法（見上方 D-010）是拿獨立人工樣本比對，本專案僅有的 12 筆人工樣本已在四方法比較中用盡。
3. 「完成標註」不等於「分類正確」——這仍然是 D-010 所述的 silver label／弱監督性質，不是人工真值；[D-007](#d-007-選樣偏差修正2026-09-13) 要求的真實代表性資料與獨立測試集需求不變。
