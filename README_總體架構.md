...
**測試組1（81份來源隔離 holdout，真值＝本輪 LLM 自己的標籤——B 對這組真值有結構性主場優勢，因為 B 就是用這組標籤的訓練切分訓練出來的，A／C／D 從未針對這套標籤調整過，比較對 A／C／D 不完全公平，僅供觀察相對模式）：**

| 方法 | 涵蓋率（有給答案） | 有答案時準確率 | 整體準確率（棄權算錯） | macro-F1（棄權算錯） | 總耗時(81份) |
|---|---|---|---|---|---|
| A 規則式 | 22%（18/81） | 100% | 22% | 0.37 | <1秒 |
| B LLM標籤訓練分類器 | 100%（81/81） | 81.5% | 81.5% | 0.70 | <1秒 |
| C 純檢索 | 33%（27/81） | 81.5% | 27% | 0.36 | 9.8秒 |
| D 檢索＋重排 | 43%（35/81） | **65.7%** | 28% | 0.36 | **546秒**（54次重排） |

**測試組2（12份既有獨立人工標籤，真值可信）：**

| 方法 | 涵蓋率 | 有答案時準確率 | 整體準確率 | macro-F1 | 總耗時 |
|---|---|---|---|---|---|
| A 規則式 | 50%（6/12） | 100% | 50% | 0.57 | <1秒 |
| B（未修正，含資料洩漏，不可信） | 100% | 100%⚠️ | 100%⚠️ | 1.0⚠️ | - |
| B（無洩漏版本，見上節查核） | 100%（12/12） | 66.7% | 66.7% | 0.62 | <1秒 |
| C 純檢索 | 67%（8/12） | 100% | 66.7% | 0.70 | 0.8秒 |
| D 檢索＋重排 | 75%（9/12） | 100% | 75% | 0.76 | 42秒（4次重排） |

過程中發現並已修正一個真實 bug：程式最初直接沿用 `models/classifier.joblib` 評估測試組2的 B，但該模型的訓練集（170份）本來就包含這 12 筆人工文件（它們的 source_id 不在本輪 23 個 holdout 名單內)，導致 100% 準確率毫無意義（模型訓練時已看過答案）；已改用先前「僅用五類 LLM 標籤訓練、完全排除這 12 筆」的無洩漏模型（即上節查核所用模型）取代，數字與上節一致（macro-F1 0.62）。

**檢索＋重排是否有效：現有證據不支持一致的正面結論。** 兩組測試出現互相矛盾的訊號：在樣本較大的測試組1，加上重排後「有答案時準確率」反而從 81.5% 掉到 65.7%（重排新接受的文件裡錯誤比例高於棄權時放棄的部分），涵蓋率雖從 33% 升到 43%，但要付出 546 秒 vs 9.8 秒（約 56 倍）的延遲代價；在樣本極小的測試組2，重排小幅提升涵蓋率（67%→75%）且未傷準確率，但 n=12 個位數樣本、42 秒 vs 0.8 秒的延遲差距同樣過大。兩組資料量都不足以下定論，但至少沒有找到「重排帶來穩定準確率提升」的證據，反而在較大的那組出現準確率下降；這與專案既有基準（[語意模型實測](docs/語意分類實測20260912.md) CPU 每份中位數 reranker 3.8 秒、[配額收集](docs/配額收集20260913.md)「不推薦全量 reranker」）方向一致。純檢索與檢索＋重排的涵蓋率（27–75%）都遠低於訓練分類器的 100%，代表這條路線的定位應是「高把握才回答、其餘交人工」的保守分流，不是取代分類器的主力方案；規則式方法涵蓋率最低但有答案時從未出錯（兩組皆 100%），適合當作零成本的高精準子集。

此比較只是同批開發診斷，測試組1的真值本身就是本輪 LLM 標的、測試組2樣本量僅 12，兩者都不能當作正式跨方法效能結論；若要確立重排是否值得其延遲成本，需要更大規模、與訓練/標籤來源脫鉤的獨立測試集。
