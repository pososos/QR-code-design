...
1. 修復／審核文字品質，擴充跨來源、跨版型的各用途樣本，補安全資料類。
2. 人工標註；依來源／文件家族切分，避免版本、近似文件、切片洩漏。
3. 訓練真實基準模型，量測各類召回率、Macro-F1、混淆矩陣、延遲；與人工規則比較。
4. 加關鍵字索引、處理政策、狀態與版本管理；分類不可成唯一檢索閘門。
5. 需求允許時再做時間軸、實體候選、局部向量化／排序及 LLM 關係抽取。
6. 整理三分鐘 QR Demo、架構說明、Git 與驗證證據。已建立 cement-crawl Skill 作爬蟲操作入口；其餘 Skill 尚未建立。
